...
A continuación, mostramos su simulación del método de montecarlo para poder obtener una estimación de $\pi$ y como comentado vemos que cuando aumentan nuestras ejecucciones el valor cada vez se aproxima más a su valor real.
...
$$ 
donde $U_i\sim \mathcal{U}(0,1))$. Además, se puede demostrar que el valor de $\hat{\alpha}_n$ converge $\alpha$ cuando $n$ converge hacia $\infty$. El principio de aletoriedad subyacente es el msmo y por tanto extrapolable que en el calculo de pi. Al tomar una distribución uniforme en el intervalo $[0,1]$, todos los puntos en dicho intervalo son equiprobables. 
...
jdm_bs(day=100,monte_carlo=2,start_price=c(5500),mu=c(0.1),sigma=c(0.11),lambda=2,K=c(6000),plot=TRUE)
valor
```
...
library(dplyr)
a=GBM(N=1000,T=1,t0=0,x0=1,theta=4,sigma=2)
b=GBM(N=1000,T=1,t0=0,x0=1,theta=4,sigma=2)
c=GBM(N=1000,T=1,t0=0,x0=1,theta=4,sigma=2)
plot(a)
par(new=TRUE)
plot(b)
plot(c)
...
server <- shinyServer(function(input, output) {
  ...
  output$oid2 <- renderPrint({
    ...
    print(as.numeric(input$vec1))
    a=as.array(as.numeric(input$vec1))
    print(a)
    print(a[1])
  })
